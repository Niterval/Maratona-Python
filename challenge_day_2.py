...
def is_on_list (serial, tracking_codes):
    return serial in tracking_codes


def add_tracking_code (add_code,tracking_codes):
    tracking_codes.append (add_code)


def remove_tracking_code (rm_code,tracking_codes):
    tracking_codes.remove (rm_code)

    # tracking_codes.remove (rm_code)
    # tracking_codes.remove (rm_code)

def get_tracking_code (item, tracking_codes):
    return tracking_codes[item]


def sum_two_lists (company_two_tracking_codes, tracking_codes):
    return len(company_two_tracking_codes +  tracking_codes)
...
def count_in_list (nun_count, last_sales):
    return last_sales.count(nun_count)
